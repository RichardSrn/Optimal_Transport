...
def collect(path="./data/", save=True, save_path="./results/"):
    ...
    indexes.to_csv(os.path.join(save_path,"indexes.csv"))
    ...
    for d in tree.keys():
        for file_name, barycenter in zip(tree[d], data[d]):
            ...
            param["center_distance"] = barycenter_loc
...
def compare_obv_thr_pixels(show_plot=True,
    ...
    def count_pixels(pixels):
        ...
        for i in range(len(pixels)):
            if str(pixels[i]) != "nan":
                new_pixels.append(int(pixels[i]))
            else:
...
def compare_center_distance(show_plot=True,
                                save_plot=True,
                                show_points_params=True):
    """
    ...
    df = pd.read_csv(os.path.join("./results", "DataFrame_summary.csv"))
    df["center_distance"] = adapted_l2(df[
                                               "center_distance"])  # /!\ here we have the true average equal to (24.5,24.5) and not (25,25) due to the rounding as integer. See generate_data_noise_grading.py line 60.
    make_plot(df=df,
              min_or_max="min",
...
def make_plot(df,
    ...
    to_be_droped = list({"center_distance",
                         "above-thld_pixels",
...
def compare_all(re_collect=True,
    ...
    if center_dist:
        ...
        compare_center_distance(show_plot=show_plots)
        print("DONE")
