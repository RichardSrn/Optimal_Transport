...
def entropic_reg_bary_2(reg = 0.04, metric = "sqeuclidean", size_x=50, size_y=50, plot=False, save=False, show=False):
    ...
    for file in files:
        ...
        np.save("./results/entropic_reg_bary_2/" + title + ".npy", bary)
    ...
    if show:
        plt.show()
    if save:
        plt.savefig("./results/entropic_reg_bary_2/entropic_"+str(reg)+"_reg_"+str(data.shape[0])+"_samples.png")


if __name__ == "__main__":
    reg=0.4
    entropic_reg_bary_2(reg=reg)
