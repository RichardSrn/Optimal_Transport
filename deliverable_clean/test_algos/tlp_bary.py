...
def tlp_bary(reg=0.1, eta=0.1, x_size=50, y_size=50, outItermax=10, weights=None, inItermax=100, outstopThr=1e-8,
    ...
    for file in files:
        ...
        bary, barys = tlp_bi(hs, hs_hat, x_size, y_size, reg, eta, weights, outItermax, inItermax, outstopThr,
                             instopThr, log=log)
        print(bary[0])
        ...
        params = "_reg_" + str(reg) + "_eta_" + str(eta) + "_outer-inner_" + str(outItermax) + "-" + str(
            inItermax) + "_samples_" + str(samples) + "_intensity_" + str(intensity)
        if save:
